...
## v4.1 reliability controls

- **Tab Status Center** is a collapsed drawer that lists every supported open AI tab, its Room binding, foreground/background location, current turn, last check, and recovery count. It also shows disconnected participants that no longer have a tab.
- **Scheduled runs** are stored in `chrome.storage.local` with independent morning and evening slots. Chrome alarms are recreated after extension startup, and each slot can start Interactive or Full Auto mode with its own topic. A scheduled run reconnects matching open provider tabs and skips safely if a Room is already active or fewer than two AI tabs are available.
- **Automatic ChatGPT/Gemini recovery** reattaches content scripts, refreshes Gemini's background lifecycle when needed, preserves the original transaction baseline, and re-arms response observation. It never clicks Send a second time for the same transaction; exhausted recovery becomes `NEEDS ATTENTION`.

Open the **Tab Status Center** or **Scheduled runs** drawers in the Side Panel. Save a schedule only after the desired Room and provider tabs are configured. **Run now** is available for a safe manual schedule check.
...
1. Download `lee-relay-bot-v4.1.5-installable.zip` from the [v4.1.5 release](https://github.com/juwonllee2024-dotcom/lee-relay/releases/tag/v4.1.5) and unzip it into a new folder.
