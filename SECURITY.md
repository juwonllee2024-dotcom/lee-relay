...
Security fixes are currently targeted at the latest `4.3.x` release.
...
Context Shelf reads only files the user explicitly selects in the Side Panel. It shows the filename, preview, size, and SHA-256 before use. Selected contents remain in the active browser session, are sent only through the chosen provider tab when a turn runs, and are removed from durable meeting storage. Context Guard can clear the selected contents after the next verified turn. Handoff receipts retain only bounded provider and file metadata, never local paths or contents. Clear the Shelf or remove individual files before switching rooms.
