(() => {
  ...
  const inputEngine = globalThis.__LEE_RELAY_INPUT_ENGINE__ || null;
  ...
  function fallbackUsableInput(input) {
    if (!input || input.isConnected === false) return false;
    const tagName = String(input.tagName || '').toLowerCase();
    if (tagName === 'textarea') return !input.disabled && !input.readOnly;
    if (tagName === 'input') {
      const type = String(input.type || input.getAttribute?.('type') || 'text').toLowerCase();
      return !['hidden', 'file', 'button', 'submit', 'reset', 'checkbox', 'radio', 'range', 'color'].includes(type)
        && !input.disabled && !input.readOnly;
    }
    const declared = input.getAttribute?.('contenteditable')?.toLowerCase();
    return declared !== 'false' && (input.isContentEditable === true || declared === '' || declared === 'true');
  }
  ...
  function findInput() {
    ...
    for (const selector of adapter.inputSelectors) {
      ...
      const selected = inputEngine?.selectUsableInput
        ? inputEngine.selectUsableInput(candidates, isVisible)
        : candidates.find((input) => isVisible(input) && fallbackUsableInput(input));
  ...
  function inputFailureReason() {
    const shell = queryDeep('#prompt-textarea', getRoots(true)).find(isVisible) || null;
    const sidePanelAvailable = document.documentElement?.getAttribute('data-chatgpt-extension-side-panel') === 'available';
    if (provider === 'chatgpt' && shell?.getAttribute?.('contenteditable')?.toLowerCase() === 'false') {
      if (sidePanelAvailable) {
        return 'ChatGPT composer is locked by the ChatGPT Side Panel integration. Disable that integration or open a normal ChatGPT composer, then reload the tab.';
      }
      return 'ChatGPT composer is read-only. Open a normal ChatGPT composer, then reload the tab.';
    }
    return `${provider} input editor not found.`;
  }

  function inputText(input) {
    if (!input) return '';
    if (inputEngine?.readText) return cleanText(inputEngine.readText(input));
  ...
  function setInputText(input, text) {
    if (inputEngine?.replaceInputText) {
      return inputEngine.replaceInputText(input, text, globalThis, { maxAttempts: 3 });
    }
    input.focus();
  ...
  async function waitForCompleteComposer(initialInput, text) {
    if (inputEngine?.waitForCompleteInput) {
      return inputEngine.waitForCompleteInput(() => findInput(), text, {
        initialInput,
        timeoutMs: 1800,
        intervalMs: 80,
        stableReads: 2,
    ...
    while (Date.now() < deadline) {
      ...
      const observed = inputText(input).replace(/\r\n?/g, '\n').replace(/\s+/g, ' ').trim();
      if (expected === observed) {
        stableReads += 1;
        if (stableReads >= 2) return { ok: true, input, text: inputText(input), stableReads };
      } else {
        stableReads = 0;
      }
      await new Promise((resolve) => setTimeout(resolve, 80));
    }
    return { ok: false, input, text: inputText(input), stableReads };
  }
  ...
  function contextAttachmentBusy(fileName = '') {
    ...
    const stateNodes = queryDeep(stateSelectors, roots);
    ...
    // A provider can put the progress state on a wrapper around the named
    // attachment. Check a short ancestor chain as well, without treating a
    // generic always-visible Upload button as an active upload.
  ...
  async function attachContextFile(message) {
    ...
    if (!composer) {
      transaction.contextFilePending = false;
      return { attached: false, ready: false, reason: inputFailureReason(), fileName };
    }
  ...
  async function submitMessage(message) {
    ...
    let input = findInput();
    if (!input) throw new Error(inputFailureReason());
    let insertionResult = setInputText(input, text);
    let settled = await waitForCompleteComposer(input, text);
    // A provider can replace its ProseMirror node while handling the first
    // paste. Re-resolve once and reinsert on that fresh node before giving up;
    // no Send action is allowed during this recovery.
    if (!settled?.ok) {
      const retryInput = findInput();
      if (retryInput) {
        input = retryInput;
        insertionResult = setInputText(input, text);
        settled = await waitForCompleteComposer(input, text);
      }
    }
    const currentInput = findInput();
    const verifiedInput = currentInput || settled?.input || input;
    ...
    const composerText = inputText(verifiedInput);
    ...
    const finalReadbackMatches = inputEngine?.matchesCompleteText
      ? inputEngine.matchesCompleteText(verifiedInput, text)
      : Boolean(composerSignature && expectedSignature && composerSignature === expectedSignature);
    const composerMatchesPrompt = Boolean(settled?.ok && finalReadbackMatches);
    if (!composerMatchesPrompt) {
      const normalizeForLength = (value) => String(value ?? '').replace(/\r\n?/g, '\n').replace(/\s+/g, ' ').trim();
      const expectedChars = normalizeForLength(text).length;
      const observedChars = normalizeForLength(composerText).length;
      const insertionReason = insertionResult?.ok === false && insertionResult.reason
        ? ` ${insertionResult.reason}`
        : '';
      throw new Error(`${provider} composer did not accept the complete relay prompt (expected ${expectedChars} chars, observed ${observedChars} chars).${insertionReason} Send was cancelled to prevent a partial or duplicate message.`);
    }
    transaction.inputPrimed = true;

    let button = findSendButton(verifiedInput);
    if (provider === 'chatgpt') {
      const sendReady = await waitForChatgptSendReady(verifiedInput);
      ...
      const sendInput = sendReady.input || verifiedInput;
      const sendText = inputText(sendInput);
      const sendMatchesPrompt = inputEngine?.matchesCompleteText
        ? inputEngine.matchesCompleteText(sendInput, text)
        : cleanText(sendText) === cleanText(text);
      if (!sendMatchesPrompt) throw new Error('ChatGPT composer changed before Send; Send was not attempted to prevent a partial or duplicate message.');
    }
    if (button) {
      button.click();
    } else {
      const form = verifiedInput.closest?.('form');
      if (form?.requestSubmit) form.requestSubmit();
      else verifiedInput.dispatchEvent(new KeyboardEvent('keydown', { bubbles: true, cancelable: true, key: 'Enter', code: 'Enter', keyCode: 13, which: 13 }));
    }
    ...
    (async () => {
      switch (message?.type) {
        case 'PING':
          {
            const input = findInput();
      sendResponse({ ok: true, provider, attached: Boolean(attachment), transactionId: transaction?.transactionId || null, inputAvailable: Boolean(input), inputError: input ? '' : inputFailureReason(), contentVersion: CONTENT_SCRIPT_VERSION });
          }
          break;
        case 'ATTACH_PARTICIPANT':
          ...
          {
            const input = findInput();
      sendResponse({ ok: true, provider, url: location.href, inputAvailable: Boolean(input), inputError: input ? '' : inputFailureReason(), contentVersion: CONTENT_SCRIPT_VERSION });
          }
          break;
  ...
  ensurePolling();
  chrome.runtime.sendMessage({ type: 'PAGE_READY', provider, url: location.href, contentVersion: CONTENT_SCRIPT_VERSION }).catch(() => {});
})();
